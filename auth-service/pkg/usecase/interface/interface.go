...
type IUserUseCase interface {
	...
	UserLogin(string, string) (*responsemodel_auth_server.AuthenticationResponse, error)
	VerifyUserToken(string) (string, error)
	SendOtp(string) (string, error)
	ForgotPassword(requestmodel_auth_server.ForgotPassword) error
	SeperateUserIDFromAccessToken(string) (string, error)
	CreateAcceesTokenByValidatingRefreshToken(string) (string, error)
